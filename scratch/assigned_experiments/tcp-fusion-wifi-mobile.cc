...
NS_LOG_COMPONENT_DEFINE("TcpFusionWifiMobileAssigned");
...
main(int argc, char* argv[])
{
    ...
    for (uint32_t i = 0; i < pairs.size(); ++i)
    {
        ...
        OnOffHelper onoff("ns3::TcpSocketFactory", InetSocketAddress(ifs.GetAddress(dst), port));
    ...
    double remEnergy = 0.0;
    for (uint32_t i = 0; i < sources.GetN(); ++i)
    {
        Ptr<BasicEnergySource> src = DynamicCast<BasicEnergySource>(sources.Get(i));
        if (src)
        {
            remEnergy += src->GetRemainingEnergy();
        }
    }
    double energyJ = nNodes * initialEnergyJ - remEnergy;
    ...
    out << std::fixed << std::setprecision(6) << "wifi-mobile"
