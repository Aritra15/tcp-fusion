#!/usr/bin/env python3
"""
run_tcp_fusion_experiments.py

Master script to run all TCP-Fusion paper experiments and generate plots.

Produces the following figures (matching the paper):
  Figure 4 — Single flow throughput vs loss rate
              (TcpFusion, TcpNewReno, TcpCubic, TcpBic, TcpHighSpeed, TcpVegas, TcpWestwoodPlus)
  Figure 5 — Two coexisting flows throughput vs loss rate
              (TcpFusion+Reno, TcpVegas+Reno, TcpWestwoodPlus+Reno)
  Figure 6 — Two coexisting flows throughput vs loss rate
              (TcpHighSpeed+Reno, TcpBic+Reno, TcpCubic+Reno)
  Figure 7 — Congestion window behaviour over time (Fusion vs Reno, two flows)

Usage:
    cd <ns-3-root>
    python3 scratch/run_tcp_fusion_experiments.py

All results are saved in scratch/tcp_fusion_results/
"""
...
SIM_TIME   = 30          # seconds per run (reduced for faster demo; paper uses 60+)
SEG_SIZE   = 1448

# Loss rates spanning 10^-6 to 10^-1 (log scale, 8 points)
LOSS_RATES = [1e-6, 1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 1e-2, 1e-1]

# TCP variants available in ns-3 for comparison
...
# Variants for two-flow fairness experiments (Figure 5-like: delay/hybrid)
TWO_FLOW_VARIANTS_FIG5 = [
    "ns3::TcpFusion",
    "ns3::TcpVegas",
    "ns3::TcpWestwoodPlus",
]

# Variants for two-flow fairness experiments (Figure 6-like: loss-based aggressive)
TWO_FLOW_VARIANTS_FIG6 = [
    "ns3::TcpHighSpeed",
    "ns3::TcpBic",
    "ns3::TcpCubic",
]
...
def run_ns3(program, args_str):
    """Run an ns-3 scratch program and return stdout (last non-empty line)."""
    ...
    # Return only the last non-empty line (avoid cmake config noise)
...
# ─── EXPERIMENT 1: Single Flow Throughput vs Loss Rate (Figure 4) ──────────
def run_single_flow():
    print("\n" + "=" * 70)
    print("EXPERIMENT 1: Single flow throughput vs loss rate")
    print("=" * 70)

    results = {}  # variant -> [(lossRate, throughput)]
    ...
    for variant in SINGLE_FLOW_VARIANTS:
        ...
        for lr in LOSS_RATES:
            ...
            print(f"    {name} loss={lr:.0e}  throughput={thr:.2f} Mbps")

    # Save raw data
    fpath = os.path.join(OUT_DIR, "single_flow_throughput.dat")
    with open(fpath, "w") as f:
        f.write("# lossRate " + " ".join(
            [v.split("::")[-1] for v in SINGLE_FLOW_VARIANTS]) + "\n")
...
# ─── EXPERIMENT 2: Two Coexisting Flows (Figures 5 & 6) ───────────────────
def run_two_flows(variants, label):
    print(f"\n{'=' * 70}")
    print(f"EXPERIMENT 2 ({label}): Two coexisting flows throughput vs loss rate")
    print("=" * 70)

    results = {}  # variant_name -> [(lr, variant_thr, reno_thr)]
    ...
    for variant in variants:
        ...
        for lr in LOSS_RATES:
            ...
            print(f"    {name}+Reno loss={lr:.0e}  variant={vthr:.2f}  reno={rthr:.2f} Mbps")
    ...
    with open(fpath, "w") as f:
        ...
        for variant in variants:
            name = variant.split("::")[-1]
            f.write(f" {name}_thr Reno_with_{name}_thr")
        f.write("\n")
        for i, lr in enumerate(LOSS_RATES):
            row = [f"{lr:.1e}"]
            for variant in variants:
                name = variant.split("::")[-1]
                row.append(f"{results[name][i][1]:.4f}")
                row.append(f"{results[name][i][2]:.4f}")
            f.write(" ".join(row) + "\n")
    print(f"  Saved: {fpath}")
    return results


# ─── EXPERIMENT 3: CWND Behaviour (Figure 7) ──────────────────────────────
def run_cwnd_traces():
    print(f"\n{'=' * 70}")
    print("EXPERIMENT 3: Congestion window behaviour over time")
    print("=" * 70)

    cwnd_time = 100  # longer for visible cwnd dynamics
    for variant in CWND_VARIANTS:
        name = variant.split("::")[-1]
        prefix = f"cwnd_{name}"
        args = (f"--tcpVariant={variant} --simTime={cwnd_time} "
...
def generate_plots(single_results, two_fig5_results, two_fig6_results):
    ...
    try:
        ...
        print("  !! Skipping plot generation. Raw data files are in", OUT_DIR)
        return
    ...
    # ── Figure 4: Single flow throughput ──
    ...
    ax.set_title("Figure 4: Single Flow Throughput vs Loss Rate", fontsize=14)
    ...
    # ── Figure 5: Two flows (hybrid/delay variants) ──
    fig, ax = plt.subplots(figsize=(10, 6))
    ...
    ax.set_xlabel("Packet Loss Rate", fontsize=13)
    ax.set_ylabel("Throughput (Mbps)", fontsize=13)
    ax.set_title("Figure 5: Two Coexisting Flows — Hybrid/Delay Variants + Reno", fontsize=14)
    ...
    # ── Figure 6: Two flows (loss-based aggressive) ──
    fig, ax = plt.subplots(figsize=(10, 6))
    ...
    ax.set_xlabel("Packet Loss Rate", fontsize=13)
    ax.set_ylabel("Throughput (Mbps)", fontsize=13)
    ax.set_title("Figure 6: Two Coexisting Flows — Loss-based Variants + Reno", fontsize=14)
    ax.legend(fontsize=9, ncol=2)
    ax.grid(True, which="both", alpha=0.3)
    ax.set_xlim([5e-7, 2e-1])
    fig.tight_layout()
            ...
            if os.path.exists(fpath) and os.path.getsize(fpath) > 0:
                try:
                    ...
                    if data.ndim == 2 and data.shape[0] > 0:
                        t = data[:, 0]
                        cwnd = data[:, 1] / 1448.0  # convert bytes to segments
                        ax.plot(t, cwnd, linestyle=style, linewidth=1.0, label=label, alpha=0.8)
                except Exception as e:
                    print(f"  Warning: could not load {fpath}: {e}")

        ax.set_ylabel("cwnd (segments)", fontsize=11)
        ax.set_title(f"CWND: {name} + NewReno coexisting", fontsize=12)
        ax.legend(fontsize=9)
        ax.grid(True, alpha=0.3)

    axes[-1].set_xlabel("Time (s)", fontsize=11)
    fig.suptitle("Figure 7: Congestion Window Behaviour", fontsize=14, y=1.01)
    fig.tight_layout()
...
def main():
    ...
    # Pre-build once so --no-build works
    print("Pre-building ns-3...")
    subprocess.run("./ns3 build", shell=True, cwd=NS3_DIR,
                   capture_output=True, timeout=300)

    print(f"NS-3 directory: {NS3_DIR}")
    print(f"Output directory: {OUT_DIR}")
    print(f"Simulation time per run: {SIM_TIME}s")
